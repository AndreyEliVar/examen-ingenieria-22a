...
import {cantidadRefrescosValida, monedasEnMaquina,  cantidadMonedasValida, realizarVerificacionPago} from "../../logic/VerificadorErrores"
...
describe("calcular total", () => {
    ...
    describe("cantidad refrescos valida", () => {
        it("si no tiene refrescos seleccionados retorna false", () => {
            ...
            const cantidad = 5;
            const resultado = cantidadRefrescosValida(refrescosSeleccionado, cantidad);
            expect(resultado).toEqual(expect.objectContaining({cantidadRefrescos: true}));
        ...
        it("Si se selecciona un fresco de cada tipo retorna true", () => {
            ...
            const cantidad = 100;
            const resultado = cantidadRefrescosValida(refrescosSeleccionado, cantidad);
            expect(resultado).toEqual(expect.objectContaining({cantidadRefrescos: false}));
        }
        );
    }
    );
    ...
    describe("cantidad monedas valida", () => {
        it("si tiene monedas retorna true", () => {
            ...
            expect(resultado).toEqual(expect.objectContaining({cantidadMonedas: true}));
        }
        );
        it("si no tiene monedas retorna false", () => {
            ...
            const totalPagar = 100;
            const resultado = cantidadMonedasValida(montoIngresado, totalPagar);
            expect(resultado).toEqual(expect.objectContaining({cantidadMonedas: false}));
        }
        );
    }
    );
    ...
    describe("realizar verificacion pago", () => {
        it("si no tiene errores retorna true", () => {
            ...
            expect(resultado).toEqual(expect.objectContaining({puedoPagar: true}));
        }
        );
        it("si tiene errores retorna false", () => {
            ...
            const totalPagar = 100;
            const resultado = realizarVerificacionPago(montoIngresado, totalPagar);
            expect(resultado).toEqual(expect.objectContaining({puedoPagar: false}));
        }
        );
    }
);
